...
public class Camera {
   ...
   public void centerOn(float x, float y) {
      ...
      cameraX = x - gc.getWidth()  / 10; //was2
      cameraY = y - gc.getHeight() / 10;
      
      //if the camera is at the right or left edge lock it to prevent a black bar
      //int widthcheck = gc.getWidth(); //this should be 160
      if(cameraX < 0) cameraX = 0;
      if(cameraX + 160 > mapWidth) cameraX = mapWidth - 160; //  if(cameraX + gc.getWidth() > mapWidth) cameraX = mapWidth - gc.getWidth();
      
      //if the camera is at the top or bottom edge lock it to prevent a black bar
      if(cameraY < 0) cameraY = 0;
      if(cameraY + 160 > mapHeight) cameraY = mapHeight - 160;  //if(cameraY + gc.getHeight() > mapHeight) cameraY = mapHeight - gc.getHeight();
   }
   ...
   public void centerOn(float x, float y, float height, float width) {
      this.centerOn(x + width / 2, y + height / 2); //was2
   }
